...
from typing import (
    Any, Dict, List, Optional, Protocol, Tuple, TypeVar, Union,
    runtime_checkable
)
...
TensorLike: TypeAlias = Union[Tensor, List[float], float]
Device: TypeAlias = Union[torch.device, str, None]
DType: TypeAlias = Optional[torch.dtype]
Shape: TypeAlias = Union[torch.Size, Tuple[int, ...], List[int]]
...
class EnergyModel(Protocol):
    ...
    def energy(self, x: Tensor, *, beta: Optional[Tensor] = None) -> Tensor:
    ...
    def free_energy(self, v: Tensor, *, beta: Optional[Tensor] = None) -> Tensor:
...
InitStrategy: TypeAlias = Union[str, float, Tensor, callable]
