...
class DeviceInfo:
    ...
    cached_memory: Optional[int] = None
...
class DeviceManager:
    ...
    @contextmanager
    def device_placement(self, device: Union[str, torch.device, None] = None) -> Iterator[None]:
    ...
    @contextmanager
    def autocast(self, enabled: bool = True, dtype: Optional[torch.dtype] = None):
...
def set_device(device: Union[str, torch.device, None]) -> None:
    """Set global device."""
    global _global_manager
...
def to_device(tensor: Tensor, device: Optional[torch.device] = None) -> Tensor:
